...
def create_style(border_cat):
    ...
    html_font = 'font-family:' + random.choice(font_list_en) + ';'
...
def transform_html_id_text(html_input):
    ...
    # randomly choice table styles (English)
    table_style = create_style(random.choice(style_list))
    ...
    return struc_tokens, table_html, list_cell_contents, idx_count


class WikiTableParser(object):
    def __init__(self, jsonl_path, save_path='', start_id=0, end_id=-1, chunks_nums=1, suffix=''):
    ...
    def read_json_tables(self, this_chunk, chunks_idx):
        ...
        for i in range(this_chunk[0]):
            ...
            if not line:
                break
        # print("chunks_idx 2 {} : {}".format(chunks_idx, this_chunk))
        for i in tqdm(range(this_chunk[0], this_chunk[1])):
            ...
            if struc_tokens is None:
                ...
                with open(self.save_path + 'errors/' + str(i) + self.suffix + 'transform_html_id_text.txt', 'w') as s_json:
            ...
            if bboxes is None:
                # save error patterns to folder
                if im is not None:
                    with open(self.save_path + 'errors_big/' + str(i) + self.suffix + '.txt', 'w') as f:
                        f.write(html_with_id)

                    im.save(self.save_path + 'errors_big/' + str(i) + self.suffix + '.png', dpi=(600, 600))
                else:
                    with open(self.save_path + 'errors/' + str(i) + self.suffix + 'html_to_img.txt', 'w') as s_json:
                        s_json.write(str(table_obj))
            ...
            if im.size[0] > MAX_WIDTH_IMAGE or im.size[1] > MAX_WIDTH_IMAGE * 1.5:
                ...
                # draw_matrices(img, np.array(bboxes), str(i) + self.suffix + '.jpg', self.save_path + 'errors_big/')

                continue
            # #########################

            im.save(self.save_path + 'table_images/' + str(i) + self.suffix + '.png', dpi=(600, 600))
            ...
            if 'aspects' in table_obj:
                ...
                aspects = ''
            table_sample = {'filename': str(i) + self.suffix + '.png',
                            'aspects': aspects,
                            'imgid': i,
                            'html': html_json}

            with open(self.save_path + 'table_images/' + str(i) + self.suffix + '.json', 'w') as s_json:
                json.dump(table_sample, s_json)

            # # ##########debug
            # with open(self.save_path + 'boxes/' + str(i) + self.suffix + '.txt', 'w') as f:
            #     f.write(html_with_id)
            #     f.write(str(table_sample))
            #
            # img = np.asarray(im, np.int64)[:, :, 0]
            # draw_matrices(img, np.array(bboxes), str(i) + self.suffix + '.jpg', self.save_path + 'boxes/')
            # #########################
            chunk_total_item += 1

        print("chunks_idx {} : {} / {} samples".format(chunks_idx, chunk_total_item, this_chunk[1] - this_chunk[0]))
        with open(self.save_path + '/' + str(chunks_idx) + self.suffix + '.txt', 'w') as f_txt:
...
if __name__ == "__main__":
    ...
    part_ = '3'
    json_file = '../html_wiki_tables/en_' + part_ + '.jsonl'

    save_path_ = '../wiki_en_tables_borderless/part_' + part_ + '/'
    ...
    parser = WikiTableParser(jsonl_path=json_file, save_path=save_path_, start_id=int(args.start_id), end_id=int(args.end_id), chunks_nums=nproc, suffix='_part_' + part_)
