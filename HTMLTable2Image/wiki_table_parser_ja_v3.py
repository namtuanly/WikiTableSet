...
from WikiTableGeneration.tools import *
import cv2
import os
import numpy as np
import random
...
# ja table v3
MAX_ROWS = 60    # max number rows in table
MAX_ROW_SPAN = 35    # max row spanning in table
MAX_COL_SPAN = 25    # max col spanning in table
MAX_WIDTH_IMAGE = 1366 * 1.5  # max table image width
...
def create_style(border_cat, border_cell):
    ...
    font_size_list = ['15', '16', '17']

    html_font = 'font-family: ' + random.choice(font_list_ja) + ';font-size: ' + random.choice(font_size_list) + 'px;'
    ...
    # random dashed border line
    border_style_list = ['solid', 'dashed', 'dotted', 'double'
                         , 'ridge', 'groove'
                         ]
    border_style_bottom = random.choice(border_style_list)
    border_style_left = random.choice(border_style_list)
    ...
    if border_cat == 0:
    # full border
        style += "td,th{padding:" + str(padding_border) + "px;" + \
                 "border-bottom:" + str(border_cell) + "px " + border_style_bottom + " black;" + \
                 "border-left:" + str(border_cell) + "px " + border_style_left + " black;}"
...
if __name__ == "__main__":
    ...
    save_path_ = '../wiki_jp_tables_full_border_v3/'
    ...
    os.makedirs(save_path_ + 'diagonal_header_cell/', exist_ok=True)
